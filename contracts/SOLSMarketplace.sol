...
contract SOLSMarketplace {
    ...
    function mintLegendary(
        ...
        string memory description,
        string memory uri
    ) external isValidProductId(product_id) isValidUnitId(_legen_param.product_unit) isValidUnitId(_legen_param.ticket_unit) isNotOwner(msg.sender)  {
        ...
        // require(nft_counter.current() < max_supply, "mintLegendary: overflow supply");

        address creator = msg.sender ;

        uint256 new_nft_id = mintNFT(msg.sender, _legen_param.ticket_available , product_id , 1, name, description, uri) ;
    ...
    function buyLegendaryAsNFT(uint256 nft_id) external isValidNftId(nft_id) isLegendary(nft_id) isNotOwner(msg.sender) {
